...
def fixed_click(mask, class_id=1):
    ...
    if len(indices) == 0:
        ...
        point_labels.append(1)
        # mx = np.max(indices, axis=0)
        # mn = np.min(indices, axis=0)
        # pt = [mn[0] + (mx[0] - mn[0]) // 2, mn[1] + (mx[1] - mn[1]) // 2]
        pt = indices[len(indices)//2]
        points.append(pt)
        point_labels.append(0)
        indices = np.argwhere(mask != class_id)
...
class JointTransform2D:
    ...
    def __call__(self, image, mask, hfc):
        #  gamma enhancement
        if np.random.rand() < self.p_gama:
            c = 1
            g = np.random.randint(10, 25) / 10.0
            ...
            image = image.astype(np.uint8)
        # transforming to PIL image
        image, mask, hfc = F.to_pil_image(image), F.to_pil_image(mask), F.to_pil_image(hfc)
        # random crop
        if self.crop:
            i, j, h, w = T.RandomCrop.get_params(image, self.crop)
            image, mask, hfc = F.crop(image, i, j, h, w), F.crop(mask, i, j, h, w), F.crop(hfc, i, j, h, w)
        # random horizontal flip
        if np.random.rand() < self.p_flip:
            image, mask,hfc  = F.hflip(image), F.hflip(mask), F.hflip(hfc)
        # random rotation
        if np.random.rand() < self.p_rota:
            angle = T.RandomRotation.get_params((-30, 30))
            image, mask, hfc = F.rotate(image, angle), F.rotate(mask, angle), F.rotate(hfc, angle)
        # random scale and center resize to the original size
        if np.random.rand() < self.p_scale:
            scale = np.random.uniform(1, 1.3)
            new_h, new_w = int(self.img_size * scale), int(self.img_size * scale)
            image, mask, hfc = F.resize(image, (new_h, new_w), InterpolationMode.BILINEAR), F.resize(mask, (new_h, new_w), InterpolationMode.NEAREST), F.resize(hfc, (new_h, new_w), InterpolationMode.BILINEAR)
            # image = F.center_crop(image, (self.img_size, self.img_size))
            # mask = F.center_crop(mask, (self.img_size, self.img_size))
            i, j, h, w = T.RandomCrop.get_params(image, (self.img_size, self.img_size))
            image, mask, hfc = F.crop(image, i, j, h, w), F.crop(mask, i, j, h, w), F.crop(hfc, i, j, h, w)
        ...
        if np.random.rand() < self.p_random_affine:
            affine_params = T.RandomAffine(180).get_params((-90, 90), (1, 1), (2, 2), (-45, 45), self.crop)
            image, mask, hfc = F.affine(image, *affine_params), F.affine(mask, *affine_params), F.affine(hfc, *affine_params)
        # transforming to tensor
        image, mask, hfc = F.resize(image, (self.img_size, self.img_size), InterpolationMode.BILINEAR), F.resize(mask, (self.ori_size, self.ori_size), InterpolationMode.NEAREST), F.resize(hfc, (self.img_size, self.img_size), InterpolationMode.BILINEAR)
        low_mask = F.resize(mask, (self.low_img_size, self.low_img_size), InterpolationMode.NEAREST)
        image = F.to_tensor(image)
        hfc = F.to_tensor(hfc)
        ...
        if not self.long_mask:
            ...
            mask = to_long_tensor(mask)
            low_mask = to_long_tensor(low_mask)
        return image, mask, hfc, low_mask
...
class ImageToImage2D(Dataset):
    ...
    def __getitem__(self, i):
        ...
        hfc_path = os.path.join(os.path.join(self.dataset_path, sub_path), 'H')
        image = cv2.imread(os.path.join(img_path, filename + '.png'))
        mask = cv2.imread(os.path.join(label_path, filename + '.png'), 0)
        hfc = cv2.imread(os.path.join(hfc_path, filename + '.png'), 0)
        ...
        image, mask, hfc = correct_dims(image, mask, hfc)
        if self.joint_transform:
            # image, mask, low_mask = self.joint_transform(image, mask)
            image, mask, hfc, low_mask = self.joint_transform(image, mask, hfc)

...
def eval_mask_slice2(valloader, model, criterion, opt, args):
    ...
    for batch_idx, (datapack) in enumerate(valloader):
        ...
        hfcs = Variable(datapack['hfc'].to(dtype=torch.float32, device=opt.device))
        # print("hfcs_shape", hfcs.shape)
        masks = Variable(datapack['low_mask'].to(dtype=torch.float32, device=opt.device))
        ...
        for j in range(0, b):
            ...
            # print(pred_i)
            # im = Image.fromarray(np.uint8(pred_i))
            # im.save(os.path.join(opt.result_path, "mask/", image_filename[j]))
            if opt.mode == "val":
                cv2.imwrite(os.path.join(opt.result_path, "mask/", image_filename[j]), pred_i*255)
            del pred_i, gt_i
            if opt.visual:

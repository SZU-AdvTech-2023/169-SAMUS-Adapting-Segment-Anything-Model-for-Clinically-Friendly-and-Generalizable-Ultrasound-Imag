...
class ImageEncoderViT(nn.Module):
    def __init__(
        ...
        self.Conv = conv_block(ch_in=3, ch_out=64)
        self.msag = MSAG(64)
    ...
    def forward(self, x: torch.Tensor, h: torch.Tensor) -> torch.Tensor:
        ...
        if h.size()[1] == 1:
            h = h.repeat(1, 3, 1, 1)  # b c h w

        # hfc = self.fft(x, rate=0.3)
        # hfc = self.patch_embed(hfc)

        cnnx = self.Conv(x)
        hfc = self.Conv(h)
        msag = self.msag(cnnx)
        msag_hfc = self.msag(hfc)
        cnnx = self.cnn_embed(cnnx + 0.5 * msag)
        hfc = self.cnn_embed(hfc + 0.5 * msag_hfc)
        cnnx = cnnx + 0.5 * hfc
        # cnnx = self.cnn_embed(x)
        x = self.patch_embed(x) # b h w c
        # h = self.patch_embed(h)  # b h w c
